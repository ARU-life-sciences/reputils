pub mod html {
    ...
    pub fn render_html(matches: &clap::ArgMatches) {
        ...
        let trim_extend =
            value_t!(matches.value_of("trim_extend"), usize).unwrap_or_else(|e| e.exit());
        let trim_next_hit =
        ...
        let dot_nmatch =
            value_t!(matches.value_of("dot_nmatch"), usize).unwrap_or_else(|e| e.exit());
